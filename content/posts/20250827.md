...
Granite Vision is a 3 billion parameters model focused on entreprise use cases with visual document understanding document content extraction and working with documents.
...
This post is about SmalDocling, a very tiny and compute-effient vision-language model doing the convertion task and the instruction task written into a prompt. The post is based on ```SmolDocling``` [research paper](https://arxiv.org/abs/2502.09927) along with some personal experiences from working with it.


## Architecture

Granite Vision was trained on 12 trillion tokens
